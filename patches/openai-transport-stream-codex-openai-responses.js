...
import { a as sha256Hex, o as sha256HexPrefix } from "./crypto-digest-CNeb2i19.js";
import { t as createSubsystemLogger } from "./subsystem-C3fiUGN1.js";
import { t as asBoolean } from "./boolean-CrriykWV.js";
import { _ as truncateErrorDetail } from "./provider-http-errors-HGLTiqMh.js";
import { r as sanitizeInlineImageDataUrl } from "./inline-image-data-url-oC-MoRLP.js";
import { F as resolveProviderTransportTurnStateWithPlugin } from "./provider-runtime-CLQOjLJ6.js";
...
import { l as resolveProviderRequestPolicyConfig } from "./provider-request-config-FsDxTAaE.js";
import { t as redactIdentifier } from "./redact-identifier-CRU-WC7p.js";
import { n as repairToolUseResultPairing } from "./session-transcript-repair-BUFGXxrq.js";
import { a as buildGuardedModelFetch, c as formatModelTransportDebugBaseUrl, d as resolveModelSseDebugMode, f as resolveOpenAIStrictToolSetting, l as emitModelTransportDebug, o as resolveModelRequestTimeoutMs, u as resolveModelPayloadDebugMode } from "./stream-BcRkg2P0.js";
import { t as event_stream_exports } from "./event-stream-MHM-_qcK.js";
import { F as resolveOpenAIReasoningEffortMap, h as isGoogleGemini3ProModel, m as isGoogleGemini3FlashModel } from "./provider-stream-shared-B4Hm1tKd.js";
...
import { applyProviderReportedUsageCost, calculateCost, createFirstStreamEventAbortController, createReasoningTagTextPartitioner, getEnvApiKey, getFirstStreamEventTimeoutHandler, getFirstStreamEventTimeoutMs, parseStreamingJson, withFirstStreamEventTimeout } from "@openclaw/ai/internal/runtime";
import { describeToolResultMediaPlaceholder, extractToolResultText, sanitizeSurrogates, stripSystemPromptCacheBoundary } from "@openclaw/ai/internal/shared";
import { clampOpenAIPromptCacheKey, convertMessages, findOpenAIStrictToolProjectionDiagnostics, isOpenAICompatibleAzureResponsesBaseUrl, isOpenAIGpt54MiniModel, isOpenAIGpt55Model, isOpenAIGpt56Model, isResponsesTextContentPartType, isResponsesTextDeltaEventType, mapOpenAIStopReason, normalizeOpenAIReasoningEffort, normalizeOpenAIStrictToolParameters, projectOpenAITools, reconcileOpenAICompletionsToolChoice, reconcileOpenAIResponsesToolChoice, resolveAzureDeploymentNameFromMap, resolveOpenAIProjectedToolsStrictToolFlag, resolveOpenAIReasoningEffortForModel, resolveResponsesMessageSnapshotCollapse, supportsOpenAIReasoningEffort } from "@openclaw/ai/internal/openai";
...
const CODEX_STYLE_ABORTED_OUTPUT_APIS = /* @__PURE__ */ new Set([
	"openai-responses",
	"openai-chatgpt-responses",
	"azure-openai-responses",
	"openclaw-openai-responses-transport",
...
function createWritableTransportEventStream() {
	const eventStream = (0, event_stream_exports.createAssistantMessageEventStream)();
	return {
		eventStream,
		stream: eventStream
	};
...
const OPENAI_CODEX_RESPONSES_PROVIDERS = /* @__PURE__ */ new Set(["openai", ...(process.env.OPENCLAW_CODEX_RESPONSES_PROVIDERS ? process.env.OPENCLAW_CODEX_RESPONSES_PROVIDERS.split(",").map(function(s){return s.trim()}).filter(Boolean) : [])]);
...
function isOpenAICodexResponsesModel(model) {
	return OPENAI_CODEX_RESPONSES_PROVIDERS.has(model.provider) && (model.api === "openai-chatgpt-responses" || model.api === "openclaw-openai-responses-transport" || model.api === "openai-responses");
}
...
function resolveOpenAICodexResponsesInstructions(model, context) {
	...
	return (usesNativeOpenAICodexResponsesBackend(model) || getCompat(model).responsesInstructions) ? OPENAI_CODEX_RESPONSES_DEFAULT_INSTRUCTIONS : void 0;
}
...
function buildOpenAIResponsesParams(model, context, options, metadata) {
	...
	const useInstructions = isCodexResponses || compat.responsesInstructions === true;
	...
	const params = {
		...
		...useInstructions ? { instructions: resolveOpenAICodexResponsesInstructions(model, context) } : {},
		...metadata ? { metadata } : {}
	};
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				await processResponsesStream(responseStream, output, stream, model, {
					firstEventTimeoutMs: getFirstStreamEventTimeoutMs(options) ?? AZURE_RESPONSES_FIRST_EVENT_TIMEOUT_MS,
					abortFirstEventStream: firstEventAbort.abort,
					onFirstEventTimeout: getFirstStreamEventTimeoutHandler(options),
					signal: options?.signal,
					authProfileId: responsesOptions?.authProfileId,
...
function getCompat(model) {
	...
	return {
		...
		responsesInstructions: compat.responsesInstructions === true,
...
//#endregion
export { stripCompletionMessagesToRoleContent as C, flattenCompletionMessagesToStringContent as S, supportsModelTools as _, coerceTransportToolCallArguments as a, applyOpenAIResponsesPayloadPolicy as b, encodeAssistantTextSignatureV1 as c, mergeTransportHeaders as d, sanitizeNonEmptyTransportPayloadText as f, isReasoningOnlyLengthAssistantTurn as g, hasOnlyAssistantReasoningContent as h, createOpenAIResponsesTransportStreamFn as i, failTransportStream as l, transformTransportMessages as m, createAzureOpenAIResponsesTransportStreamFn as n, createEmptyTransportUsage as o, sanitizeTransportPayloadText as p, createOpenAICompletionsTransportStreamFn as r, createWritableTransportEventStream as s, buildOpenAICompletionsParams as t, finalizeTransportStream as u, canonicalizeMaxTokensParam as v, resolveOpenAIResponsesPayloadPolicy as x, resolveMaxTokensParam as y };
